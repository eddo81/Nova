...
class AssetManager {
  ...
  /**
   * @param string $theme_asset_manifest_path The uri of the theme's asset_manifest.json file.
   * @return self 
   */
  public function __construct(string $asset_manifest_path) {
    ...
    add_action('wp_head', function () {
      ...
      if($preloaded_assets = $this->preloaded_assests ?? '') {
  ...
  private function init_asset_loader() : void {
    foreach($this->get_theme_assets() as $asset_type => $assets) {
      ...
      foreach($assets as $asset) {
        ...
        if($asset['resource_hint'] !== 'preload') {
          continue;
        }

        $href = get_template_directory_uri() . $asset['path'];
        $file_time = filemtime(get_template_directory() . $asset['path']);
        $crossorigin = (array_key_exists('crossorigin', $asset) && ($asset['crossorigin'] === true));
  ...
  public function set_resource_hint(string $rel, string $mime, string $href, string $as, bool $cors = false) : void {
    ...
    $as = ($as) ? 'as="'. $as .'"' : null;
    $crossorigin = ($cors === true) ? "crossorigin " : null;
   ...
   * @return array Returns an array of objects which hold descriptive metadata about the theme's assets. 
   */
   ...
   * @return array Returns an array of asset names - ***handles*** - for all registered theme ***scripts*** and ***styles***.
   */
